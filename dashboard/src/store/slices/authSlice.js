...
export const forgotPassword = (email) => async (dispatch) => {
  ...
  try {
    await axiosInstance
      .post("/auth/password/forgot?frontedUrl=http://localhost:5174", email)
      .then((res) => {
    ...
    toast.error(
      error.response.data.message || "Cannot request for forgot password.",
    );
  }
};
...
export const resetPassword = (newData, token) => async (dispatch) => {
  ...
  try {
    await axiosInstance
      .put(`/auth/password/password/reset/${token}`, newData)
      .then((res) => {
